...
def extract_metrics(full_test_results_file):
    with open(full_test_results_file, 'r') as f:
        data = json.load(f)
...
def print_summary(comp_stats, suff_stats):
    ...
    print("\n" + "="*70)
    print("COMPREHENSIVENESS AND SUFFICIENCY ANALYSIS SUMMARY (FULL TEST)")
    print("="*70)
...
def main():
    full_test_results_file = Path(__file__).parent / 'results' / 'full_test_results.json'
    output_dir = Path(__file__).parent / 'plots' / 'full_test'
    output_dir.mkdir(exist_ok=True, parents=True)
    
    print("📊 Extracting metrics from full_test_results.json...")
    comp_scores, suff_scores = extract_metrics(full_test_results_file)
